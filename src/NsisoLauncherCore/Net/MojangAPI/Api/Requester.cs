...
namespace NsisoLauncherCore.Net.MojangApi.Api
{
    ...
    public static class Requester
    {
        ...
        public static string ClientToken
        {
            get
            {
                if (string.IsNullOrWhiteSpace(_clientToken)) _clientToken = Guid.NewGuid().ToString("N");
                return _clientToken;
            }
            ...
            try
            {
                ...
                if (authenticate && endpoint.Arguments.Count > 0)
                {
                    ...
                    Client.DefaultRequestHeaders.Authorization =
                        new AuthenticationHeaderValue("Bearer", endpoint.Arguments[0]);
        ...
        internal static async Task<Response> Post<T>(IEndpoint<T> endpoint)
        {
            ...
            try
            {
                ...
                var contents = new StringContent(endpoint.PostContent, Encoding, "application/json");
        ...
        internal static async Task<Response> Post<T>(IEndpoint<T> endpoint, Dictionary<string, string> toEncode)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    var err = JObject.Parse(rawMessage);
                    error = new Error
                    {
        ...
        internal static async Task<Response> Put<T>(IEndpoint<T> endpoint, FileInfo file)
        {
            ...
            try
            {
                ...
                {
                    contents.Add(new StringContent(bool.Parse(endpoint.Arguments[1]) ? "true" : "false"), "model");
