...
namespace NsisoLauncherCore.Util
{
    public static class FileHelper
    {
        ...
        public static bool IsLostJarCore(LaunchHandler core, Version version)
        {
            ...
            {
                var jarPath = core.GetJarPath(version);
        ...
        public static List<DownloadTask> GetLostAssetsDownloadTaskAsync(LaunchHandler core, Version ver)
        {
            ...
            if (!File.Exists(assetsPath))
            {
                ...
                {
                    var jsonUrl = ver.AssetIndex.URL;
        ...
        public static bool IsLostAnyLibs(LaunchHandler core, Version version)
        {
            ...
            {
                var path = core.GetLibraryPath(item);
        ...
        public static Dictionary<string, Library> GetLostLibs(LaunchHandler core, Version version)
        {
            var lostLibs = new Dictionary<string, Library>();
            ...
            foreach (var item in version.Libraries)
            {
                ...
                if (lostLibs.ContainsKey(path))
                    continue;
                if (!File.Exists(path)) lostLibs.Add(path, item);
            }
        ...
        public static bool IsLostAnyNatives(LaunchHandler core, Version version)
        {
            ...
            {
                var path = core.GetNativePath(item);
                if (!File.Exists(path)) return true;
            }
        ...
        {
            var lostNatives = new Dictionary<string, Native>();
            ...
            foreach (var item in version.Natives)
            {
                ...
                if (lostNatives.ContainsKey(path))
                    continue;
                if (!File.Exists(path)) lostNatives.Add(path, item);
            }
        ...
        private static bool IsLostAnyAssetsFromJassets(LaunchHandler core, JAssets assets)
        {
            ...
            {
                var path = core.GetAssetsPath(item.Value);
                if (!File.Exists(path)) return true;
            }
        ...
        {
            var lostAssets = new Dictionary<string, JAssetsInfo>();
            if (assets == null) return lostAssets;
            foreach (var item in assets.Objects)
            {
                var path = core.GetAssetsPath(item.Value);
                if (!lostAssets.ContainsKey(path) && !File.Exists(path)) lostAssets.Add(path, item.Value);
            }
