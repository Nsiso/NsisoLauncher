...
namespace NsisoLauncherCore
{
    ...
    public static class PathManager
    {
        ...
        ///     启动器当前真实目录
        /// </summary>
        public static string CurrentLauncherDirectory => Environment.CurrentDirectory;
        ...
        public static string BaseStorageDirectory
        {
            ...
            set => _baseStorageDirectory = value;
        }

        public static string TempDirectory => BaseStorageDirectory + "\\temp";
        ...
        /// <returns></returns>
        public static string GetGameVersionRootDir(bool versionIsolation, string gameRootPath, Version ver)
        {
            if (versionIsolation)
        ...
        {
            var artifact = new Artifact(artifactStr);
        ...
        public static string GetArtifactPath(string gameRootPath, Artifact artifact)
        {
            if (string.IsNullOrEmpty(artifact.Classifier))
                return string.Format(@"{0}\libraries\{1}\{2}\{3}\{2}-{3}.{4}",
                    gameRootPath, artifact.Package.Replace(".", @"\"), artifact.Name, artifact.Version,
                    artifact.Extension);
            return string.Format(@"{0}\libraries\{1}\{2}\{3}\{2}-{3}-{4}.{5}",
                gameRootPath, artifact.Package.Replace(".", @"\"), artifact.Name, artifact.Version, artifact.Classifier,
                artifact.Extension);
        }
        ...
        public static string GetNativePath(string gameRootPath, Native native)
        {
            return string.Format(@"{0}\libraries\{1}\{2}\{3}\{2}-{3}-{4}.jar",
                gameRootPath, native.Artifact.Package.Replace(".", @"\"), native.Artifact.Name, native.Artifact.Version,
                native.NativeSuffix);
        }
        ...
        /// <param name="ver">版本</param>
        /// <returns></returns>
        public static string GetJarPath(string gameRootPath, Version ver)
        {
            if (ver.Jar != null)
        ...
        {
            return string.Format(@"{0}\assets\objects\{1}\{2}", gameRootPath, assetsInfo.Hash.Substring(0, 2),
                assetsInfo.Hash);
        }
        ...
        public static string GetVersionOptionsPath(bool versionIsolation, string gameRootPath, Version version)
        {
            var verRoot = GetGameVersionRootDir(versionIsolation, gameRootPath, version);
